...
The menus are one column of cards — one decision per card — under a title bar that keeps the way back at the top and over an action bar that keeps **DEPLOY** at the bottom, neither of which scrolls away. The **MAP** card carries the district's own picture, drawn straight from its level file (one pixel per tile), so you can tell the maps apart before you fly one in; **MODE** is a card per mission type; and the settings that are values rather than choices — difficulty, lighting, touch controls — stay `◂ VALUE ▸` rows in the setup card. The iPhone build ([ios/](ios/)) is laid out the same way. You only see that menu once a session, though: take a district and **NEXT LEVEL** drops you straight into the next one — mission report, one press, and you are fighting, with nothing to pick in between. Lose one and you land back on the menu, where the difficulty is.

## Multiplayer

Hit **MULTIPLAYER** on the start screen, pick your name — one is already dealt for you; type over it or hit 🎲 for another — and enter the lobby. If someone in the lobby is already flying under that name you get the next one free — `ACE` becomes `ACE 2` — rather than being sent back to the name box. Matches are staged in **rooms**: create one or join one from the list — and if there's only one room going and it has space, you walk straight into it — each room runs its own match, so several groups can fight in parallel on one server. Inside a room, pick a side — **JOIN BLUE** or **JOIN RED**, up to **5 pilots per team** — and once both teams have at least one pilot, anyone on a team can hit **START MATCH** — that one press deploys everybody, so the match begins as soon as the last pilot has loaded in. The pilot who **created** the room picks the **MAP** everyone fights on (it starts on the level they had selected; the XL maps at the end of the list are sized for full 5v5 battles) — joiners see the choice in the room and in the room list before they join.
...
🌙 **LIGHTING** in the menu picks between **NIGHT MODE**, which the game defaults to, and **DAY MODE** — the district in daylight, everything on the map visible. Night mode trades that god's-eye view for sensors: night falls over the district, your mech's own lamp is most of the light there is — a cone thrown as far as your sensors reach, swinging where you turn and casting the shadows of everything it finds — and enemy mechs and turrets are only as solid as your sensors' hold on them. The turrets **you** build light their own ground, so a defended corner stays readable after you have walked away from it; the enemy's give away nothing. And the lamp is only as good as the mech carrying it: every hit browns it out and closes the beam down, so a fight you are losing is one you can see less and less of — it comes back as your mech repairs itself. Your sensors don't care about the damage, only your eyes do. Their running lights give them away in the dark before their hulls do. Contact is a strength, not a switch — a mech half behind a wall or right out at the edge of sensor range is a steady half-visible silhouette rather than something blinking on and off, it materialises quickly when it steps into the open, and it dissolves slowly when it breaks contact, so ducking behind a pillar for a moment never makes anyone disappear. When you do lose one, the minimap keeps a hollow blip where you last had it for a few seconds: you lose the mech's position, not the memory of it. An enemy shooting from cover stays hidden — its muzzle flash and tracers only appear once the shot clears the corner, so nothing gives away a mech you can't see. Bases and your own team are always visible. It's a view restriction, not a rule: it only ever hides things from the pilot who has it on, so it works in multiplayer too — but a match with one side at night and the other in daylight would be one district in two kinds of weather, so in a lobby room it sits on a **CONDITIONS** card next to the map and the mode and the room's creator sets it for everybody (rooms fought at night are marked 🌙 in the room browser). In single player it's your own call, remembered between sessions — **night mode is how the game starts**, and day mode is the setting you go and pick.
