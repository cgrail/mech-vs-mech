...
This is a map swap, not a game reset: it is only safe from the `menu` state, and every path out of a finished game still reloads. Two things ride on it — the level select keeps `?level=` in sync with `history.replaceState` (REDEPLOY is a bare `location.reload()`), while the lobby deliberately does not (the room's map is server state; a refresh drops you back on your own map). If you add module-scope state derived from `LEVEL`, either derive it live or reset it in `switchMap`.
