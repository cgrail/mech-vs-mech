...
The overlay screens are one vertical column of titled cards — one decision per card — and the layout rule is not cosmetic: it is what makes them survive a small screen. **A menu grows down, never across.** `#overlay` scrolls vertically (on iOS the card column scrolls between a pinned title bar and a pinned action button), so height is free; width is not, and a row that overflows has nowhere to go. Concretely:

- One vertical column, `min(430px, 92vw)` on web / `520` max on iOS — the same width for every card, the level list, the mode-select buttons, DEPLOY and the lobby's room rows, so the whole overlay reads as one stack.
