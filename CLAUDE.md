...
### Menus are one column of cards, and nothing grows sideways

Both builds draw a screen as the same three-piece frame — a pinned nav bar holding the way back, a scrolling column of titled cards (one decision per card), and a pinned footer holding the one green action: `LobbyChrome` / `LobbyNavBar` / `BigActionButton` in `UI/LobbyStyles.swift`, `.navBar` / `.screenScroll` / `.screenFoot` in [style.css](style.css), assembled per screen in [index.html](index.html). **Back is at the top and go is at the bottom, and neither one scrolls** — a briefing card can grow as long as it likes without pushing either off the screen. The layout rule inside the column is not cosmetic either: it is what makes a menu survive a small screen. **A menu grows down, never across.** The column scrolls, so height is free; width is not, and a row that overflows has nowhere to go. Concretely:

- One vertical column, `--col` = `min(430px, 92vw)` on web / `520` max on iOS — the same width for every card, the level list, the mode-select buttons, DEPLOY and the lobby's room rows, so the whole overlay reads as one stack. On web everything in a screen's scroll and footer takes that width from the one variable; the map editor is the single screen that widens it (for the canvas), and only for the part that scrolls.
- The overlay is measured **border-box** (`#overlay *`), so a full-width row inside a padded card stays inside it. Without that the `◂ ▸` steppers hung over the card's edge — the HUD is deliberately left content-box.
- **Never more than three controls in a row.** The two-column exception is the pair of team cards in a lobby room, which stack on a narrow phone.
- The split between the two kinds of control is the point. A **choice you want to see** is a card you tick: the map (with its own picture), the mode, the team, a level — `addHero` / `addPickCards` in [game/ui/menu.js](game/ui/menu.js), `MapHeroCard` / `ModeCard` / `CheckBadge` in `UI/LobbyStyles.swift`. A **value you cycle** stays `LABEL · VALUE` between `◂ ▸` steppers (`addOption`, `CardOptionRow`) — difficulty, controls, fog of war. One button per value is what used to overflow a row; cycling in place is what keeps a setting to one row, and it is what makes `← →` mean the same thing everywhere.
- One **green** action per screen (`.goBtn` / `BigActionButton`): DEPLOY, START MATCH, ENTER LOBBY, NEXT MAP, PLAY. Everything else is the quiet navy button. Gold is for values and titles, not for buttons. The one place the action is offered twice is the callsign card, where an on-screen keyboard covers the pinned footer — the copy that rides with the text field is a *quiet* button, so the screen still has one green one.
- Squared off (2px radius) with the lit top-left bevel on web; iOS chamfers two corners of a card (`AngledRect`) because a shape there can draw its own outline, which CSS `clip-path` cannot without eating the focus ring.
- A **map list is one function**, `addMapRow` in [game/ui/menu.js](game/ui/menu.js) (`.mapList` in [style.css](style.css)): the level select and the lobby's map picker are the same rows, opened the same way — by tapping the map hero — so picking a district behaves identically in single player and in a room, as it already did on iOS (`levelRow` / `mapListRow`). A dropdown in one of them is what that replaced.
- Map pictures come from the level text itself — one pixel per 8×8 tile, scaled blocky ([game/ui/thumb.js](game/ui/thumb.js), `MapThumbs` in `UI/LobbyStyles.swift`). No artwork ships for the 60-odd districts and none is needed. The palettes in those two files are the same list twice; change both or the same map looks different on a phone. A picture that small has to be scaled **up** to its box — `object-fit: contain` on web, `.resizable().aspectRatio(.fit)` on iOS — or a map card shows a stamp in a black field; `contain` on both sides is what keeps a tall district letterboxed rather than stretched. On iOS the pictures are drawn on a background queue and the lists that hold them are `LazyVStack`s: sixty districts' worth of CoreGraphics on the main thread stalled the run loop, which reads as the menu *and* the map orbiting behind it freezing. Anything else that slow on a menu path belongs off the main thread too — `AudioEngine.startMusic` (audio session + a 400 KB decode) is the other one.
- The small-screen media query shrinks **heights** and type, never the width.

Keyboard navigation falls out of the same structure: **selection is DOM focus**, so the keyboard, the mouse and touch can never disagree about what is highlighted. `↑ ↓` walk the visible screen's controls (steppers are skipped — they are what `← →` drive), `← →` press the focused row's own steppers, `Enter`/`Space` is the browser's own button activation. A `MutationObserver` on `.screen` class changes focuses the first row of whatever screen just opened, so every path in (flow.js, lobby.js, editor.js) gets it without calling in — "first row" being the first one *in the scrolling column*, not the nav bar's back button that precedes it in the DOM, and the row a list is already pointed at (`.selected`, the level select's current district) if there is one, which is also what scrolls that row into view; walking into a lobby room is the one screen-less change that moves the cursor itself, onto the team cards. The highlight styles `:focus`, deliberately not `:focus-visible`: that focus is moved programmatically, and a menu cursor that is sometimes not drawn is worse than one that lingers after a click.
...
- [server/server.js](server/server.js) is a dumb lobby + relay — it never simulates the game. It's hardened for internet deployment (CSP + security headers, WS origin check, connection caps, rate limits, 4 KB `maxPayload`) with env knobs `TRUST_PROXY`, `ALLOWED_ORIGINS`, `MAX_CLIENTS`, `MAX_CONNS_PER_IP` — see the README's deployment section. TLS termination and restarts belong to the platform, not the server. The CSP inline-script hashes are computed from `dist/index.html` at startup, so new inline scripts in [index.html](index.html) keep working — but any new *external* resource (CDN script, remote font) needs a CSP update. Lobby: `join`(name — a callsign somebody in the lobby is already on is **suffixed** (`ACE` → `ACE 2`), never refused; the client adopts `joined.name`, and `joined.renamed` is what makes it say so) → `createRoom`/`joinRoom` (matches are staged per room, so several run in parallel; empty rooms are deleted; if the *first* room list a client sees after joining holds exactly one room with space in it, both clients walk straight into it rather than showing a browser with one entry — armed once per lobby entry, so leaving a room never pulls you back into it) → `setLevel` / `setMode` / `setFog` (the room's **creator** picks the map, the mode — assault or ctf — and the weather; the server holds all three on the room, validates the map against its own bundle, and hands the room to the longest-standing member if the owner leaves) → `team`(blue/red/null, max 5 per side per room) → `startMatch` (anyone on a team in the room, once both sides have ≥1 pilot); the server mints a match from that room's teams (one token per player, the **room's** map, mode and fog — not the starter's) and every rostered client reloads into it, `rejoin`s by token, and starts on an all-ready `ready` → `go` handshake — **`ready` is sent by the rejoin itself, not by a button**: START MATCH was the decision, and a second per-client DEPLOY only held the roster up. The one thing that bought was a user gesture on the freshly reloaded page, so `audioCtx()` now resumes a suspended context on the first click or key instead ([game/ui/lobby.js](game/ui/lobby.js) drives all of this UI). A pre-start sweep forfeits slots that never reconnect so the handshake can't deadlock. When a match ends, `nextMatch` (the end screen's NEXT MAP) mints a **follow-up match from the finished one**: same roster minus whoever already left, next map in the server's bundle order, new tokens, delivered as a `matchStart` over the *finished* match's still-open sockets — so a rematch needs no lobby round trip. The first request mints it (`match.next`), later ones re-send it — which is what makes the **10-second countdown** on the end screen safe: every client fires `nextMatch` at roughly the same moment and they all land in the one match the first request created. Nobody has to press anything to keep a session going; the button is for whoever wants it sooner and BACK TO LOBBY for whoever wants out. The dead end is handled the same way round: once there is no follow-up match to be had — everyone else disconnected (which is also how such a match *ends*), the server refusing, or the socket dropping — NEXT MAP is a button that can only fail, so `noNextMap` drops it, makes BACK TO LOBBY the green action, and counts *that* down instead (5 s; not when it is the server that went missing, since the lobby is unreachable too). Clients spot the empty roster themselves — the peerLeft bookkeeping runs for the whole match, not just the boot handshake — so they never count down into a refusal they can already see coming. `relay` messages fan out to every other player in the match, **stamped server-side with the sender's `from` playerId** — clients trust `from`, never a sender id inside the payload.
...
- Rules: touch the enemy flag to carry it, reach your own stand to score (whether or not your own flag is home — arcade rules, no stalling), dying drops it, touching your own dropped flag returns it, and an untouched drop goes home after 25 s. `CAPTURES_TO_WIN` captures win, and they are the **only** win: `killEntity` (`projectiles.js` / `Projectiles.swift`) skips `endGame` for a base in ctf, so a base can still be levelled — which stops that side's waves, since `updateWaves` needs a live base — without ending the match. Both builds must agree on that, or one client walks out of a match the other is still playing
...
The **walk animation** hangs off the same measurement. `helpers.animateWalk(e, dt, rate)` (`Helpers.swift`) is the only thing that poses a leg — never its input, its steering heading or a peer's `moving` flag. All three walkers go through it (`player.js`, `ai.js`, `remote.js` and their Swift twins), which is also why `e.model.legL/legR` are written in exactly one place. It does three things:

- measures how far the walker is **getting**, not how far it moved. `e.anchorX/anchorZ` is an eased "where I was a moment ago" (`STRIDE_LAG`); walking drags it a fixed distance behind (speed / `STRIDE_LAG`, so `STRIDE_MIN` = 0.55 units of lag is ~4.4 u/s), while anything that *wobbles* leaves it in the middle of the wobble. A per-frame delta cannot tell those apart — a mech wedged between a wall and its own base is shoved both ways every frame and covers plenty of ground while going nowhere, which made a standing mech twitch. A lag past `STRIDE_JUMPED` is a spawn, a map switch or a replica's snap, and resets the anchor instead. `e.velX`/`velZ` (enemy aim lead, and the wire) fall out of the same vector, smoothed.
- eases an amplitude `e.stride` 0↔1 that scales the whole pose plus the caller's body bob, and advances `walkPhase` only while walking. Intent-driven animation striding on the spot is what that replaced: a mech leaning into a wall, a joystick whose release was missed, a replica whose packets stopped — and, because the amplitude eases rather than switching, legs frozen mid-stride the moment a mech stopped to shoot.
- splits the lag vector into the mech's own frame (`e.strideF`/`e.strideL`, held from the last moving frame so the ease-out keeps its direction). Fore-aft is the `rotation.x` stride and runs *backwards* when the mech backs up; sideways is a `rotation.z` shuffle where both feet reach toward the direction of travel half a cycle apart, so strafing no longer marches forward on the spot. A diagonal blends the two.

The amplitude is the return value, so bob and pose can never disagree.
...
[world/scene.js](game/world/scene.js) ↔ `GameEngine.swift`'s camera setup are one decision made twice: the web renderer runs **ACES filmic tone mapping**, SceneKit runs `wantsHDR` with a restrained bloom. Both roll the top end off, which is why the lights look over-bright on paper (hemisphere 1.15, sun 2.1 at exposure 1.25) — **change the lights and the tone mapping together, or the district goes flat/blown out**. Anything that must stay hot despite tone mapping (tracers, sparks, muzzle flashes) sets `toneMapped: false`.

That pairing is why a look is a **table, not a handful of dimmed lights**: `LOOK` in scene.js ↔ `DAY_LOOK`/`NIGHT_LOOK` in `GameEngine.swift`, one row holding the sky gradient, the fog colour, both lights, their shadow switch and the exposure, applied in one call (`setNight` / `applyLook`). `day` is the district as it always looked; `night` is what fog of war drops it into (below) — dimmed *and* opened up a third of a stop, because lights pulled down without the exposure coming up give a muddy district rather than a dark one. Nothing outside those two functions may touch a light.

Two more pieces of the look are shared: the sky is a vertical gradient drawn behind everything (a plain background texture, so it costs one quad and never moves when a map switch flies the scene — both looks' gradients are built once and swapped), and `scene.fog` takes that gradient's **horizon colour** so distant terrain melts into it. Muzzle flashes come out of a fixed ring of 28 sprites/billboards (`spawnFlash`) — a dozen shooters at several shots a second must not allocate. The vignette is pure CSS (`#hud.active::before`) / a SwiftUI gradient, never a render pass.
...
`systems/vision.js` (`Engine/Vision.swift`) is the optional fog of war — **called NIGHT MODE everywhere a player can read it** (the menu row is `🌙 LIGHTING` · `DAY MODE`/`NIGHT MODE`), while the code, the stored key `mechFog` and the wire all still say fog; keep that split, and don't let the player-facing word leak back into the identifiers or the protocol. With `game.fogOfWar` on it drops the district into the `night` look, tightens the render fog, hangs a lamp on the mech and dims out enemy mechs/turrets the sensors don't have, marking each with `e.seen` (the minimap reads it). `applyFog` is the one call that lands all of that, so the switch is one change of weather. It is a **local view restriction** — never affecting simulation, only what its own client draws — which is why it is allowed in PvP where difficulty scaling is not. It is still **the room's setting, not the pilot's**: a match where one side fights at night and the other in daylight is one district in two kinds of weather, so the room's **creator** picks it exactly like the map and the mode (`setFog` in `server.js`, the CONDITIONS card in `lobby.js` / `viewCard` in `LobbyView.swift`), it rides the match credentials as `MP.fog`/`MPConfig.fog`, and `state.js` reads it there rather than from `localStorage` whenever `MP.active`. `mechFog` is the single-player answer alone.

Three rules stop it reading as a glitch rather than as fog, and all three have to hold on either side of a new feature:

- **The district goes dark; it is not the contacts that are dimmed.** Fading enemies out of a sunlit district was the first version and read as a rendering bug. So `applyFog` switches the whole look to `night` and mounts a **sensor lamp** on the mech — a shadow-casting spot at the sensor block reaching exactly `VISION_R`, plus a dim omni so the mech is a machine rather than a silhouette. Three things keep it honest: the lamp rides **its own node**, not the player's, because that one leaves the scene for the four seconds a dead mech is waiting to respawn (a lamp left where it fell reads as the wreck still burning, and the scene's light count never changes mid-match); it is **built on first use**, so a daylight district never pays for lights it cannot see; and the night sun **hands its shadow map over** rather than adding one, so the district still renders exactly one shadow pass. The falloff is deliberately gentler than physical (`decay` 0.6 / `attenuationFalloffExponent`) — grazing incidence from a lamp 6 units up does most of the work already, and a physical curve leaves the rim of the sensor circle too dark to play. The lamp also **reads the mech's hp** (`HURT` / `hurtLamp`): hits brown the beam out and close its cone down toward a third of full, easing rather than stepping so it reads as damage instead of as a rendering fault, and the self-repair brings it back. Only the *light* — `VISION_R` and the sweep are untouched, so what you can make out changes and what anyone knows does not, which is the same reason the whole feature is allowed in PvP. **Your own turrets are lit too**, and the same light-count rule is what shapes how: a player can build turrets all match, so the lamps are a **fixed pool** (`TLAMP.pool`) lent to the nearest friendly turrets and **dimmed to zero rather than hidden or removed** when unposted — three.js recompiles every lit material when the number of lights changes. A lamp is only re-posted once it is dark, so light never teleports across the district. Enemy turrets get none: an emplacement lighting itself up would hand over the intel the fog exists to withhold.
- **Contact is a strength, not a flag.** `e.contact` is 0…1 and everything else follows from that, because a boolean is what made an earlier version strobe: one ray to one point flips as it clips a corner. Five rays per target (three up the body, two straddling it at shoulder width) make the fraction, the outer `FALLOFF` units of the sensor circle fade it rather than cutting it, and `e.fade` chases it fast in (`FADE_IN`) / slow out (`FADE_OUT`, after a `HOLD` of sensor lock) so a target that ducks behind a pillar never disappears at all. The sweep is the expensive part and runs on its own budget (`TICK`); the fade runs every frame, which is also what hides the sweep's coarseness. Web sets `opacity` per material (models build their own, so it is safe per entity) and only turns `transparent` on *while* a fade runs; iOS just sets `node.opacity`, which cascades. An entity created between two sweeps has no verdict yet: teammates and bases default visible, enemies default hidden, so nothing ever flashes into view. Losing a contact costs its position, not the memory of it — the sweep marks where it was last solid and the web minimap keeps that as a hollow blip for `GHOST` seconds (`e.ghost`; iOS has no minimap, so no mark).
