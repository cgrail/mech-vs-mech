...
**Multiplayer no longer trusts the iOS copy**, because a phone can be running an App Store build older than the deployed web game: at match boot iOS fetches the match's map from the lobby server (`GET /level/<param>` — a route beside the static files in [server/server.js](server/server.js), which resolves the param against the *deployed* `dist/levels/levels.txt` and returns that one level's text) and plays that text, falling back to its bundled copy only if the server can't be reached. Browsers need nothing here: they already load the bundle from that same server. So a stale copy no longer splits a match across two maps — but it still decides what single player, the level-select menu and the map preview show, and an iOS *starter* can still only name a level its own bundle has (the server 404s an unknown name and that client drops back to its copy). Copy it anyway.
