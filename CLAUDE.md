...
The **one** exception is the lobby's map preview: while you sit in a multiplayer room, the map orbiting behind the overlay is the room's, swapped in without a reload by `previewMap` in [game/ui/lobby.js](game/ui/lobby.js) — `rebuildWorld` (world.js) re-parses into the same `LEVEL`/`ARENA`/grid and swaps the terrain group, then the bases, marker turrets and the player mech are moved onto the new ground. Nothing else may rely on that path: it runs only in `menu` state, keeps no URL param (a refresh puts the page back on its own level), and the match still boots from a fresh load into `?level=…&mp=1`. If you add module-scope state derived from `LEVEL`, either derive it live or reset it there too.
...
- [server/server.js](server/server.js) is a dumb lobby + relay — it never simulates the game. It's hardened for internet deployment (CSP + security headers, WS origin check, connection caps, rate limits, 4 KB `maxPayload`) with env knobs `TRUST_PROXY`, `ALLOWED_ORIGINS`, `MAX_CLIENTS`, `MAX_CONNS_PER_IP` — see the README's deployment section. TLS termination and restarts belong to the platform, not the server. The CSP inline-script hashes are computed from `dist/index.html` at startup, so new inline scripts in [index.html](index.html) keep working — but any new *external* resource (CDN script, remote font) needs a CSP update. Lobby: `join`(name) → `createRoom`/`joinRoom` (matches are staged per room, so several run in parallel; empty rooms are deleted) → `setLevel` (the room's **creator** picks the map; the server holds it on the room, validates it against its own bundle, and hands the room to the longest-standing member if the owner leaves) → `team`(blue/red/null, max 5 per side per room) → `startMatch` (anyone on a team in the room, once both sides have ≥1 pilot); the server mints a match from that room's teams (one token per player, the **room's** map — not the starter's) and every rostered client reloads into it, `rejoin`s by token, and starts on an all-ready `ready` → `go` handshake ([game/ui/lobby.js](game/ui/lobby.js) drives all of this UI). A pre-start sweep forfeits slots that never reconnect so the handshake can't deadlock. `relay` messages fan out to every other player in the match, **stamped server-side with the sender's `from` playerId** — clients trust `from`, never a sender id inside the payload.
