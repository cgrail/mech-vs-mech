...
The **walk animation** hangs off the same measurement. `helpers.animateWalk(e, movedX, movedZ, dt, rate)` (`Helpers.swift`) is the only thing that poses a leg, and it takes the XZ travel the walker *actually covered this frame* — never its input, its steering heading or a peer's `moving` flag. All three walkers go through it (`player.js`, `ai.js`, `remote.js` and their Swift twins), which is also why `e.model.legL/legR` are written in exactly one place. It does three things:

- advances `walkPhase` only above `STRIDE_MIN_SPEED`, and eases an amplitude `e.stride` 0↔1 that scales the whole pose plus the caller's body bob. Intent-driven animation striding on the spot is what that replaced: a mech leaning into a wall, a joystick whose release was missed, a replica whose packets stopped — and, because the amplitude eases rather than switching, legs frozen mid-stride the moment a mech stopped to shoot.
- splits the travel into the mech's own frame (`e.strideF`/`e.strideL`, held from the last moving frame so the ease-out keeps its direction). Fore-aft is the `rotation.x` stride and runs *backwards* when the mech backs up; sideways is a `rotation.z` shuffle where both feet reach toward the direction of travel half a cycle apart, so strafing no longer marches forward on the spot. A diagonal blends the two.
- leaves the amplitude as its return value, so bob and pose can never disagree.

The player's `velX`/`velZ` (enemy aim lead, and the replica extrapolation over the wire) are measured the same way.
